...
and the adaptation procedure has been tested on numerous systems.   For
these reasons:
...
return the manual and diskettes IN THEIR UNOPENED ENVELOPE.     Once the
...
imperative that the user read and fully understand them          before
...
If you    are   missing   any of these items, contact our order department
immediately.
...
This   manual contains all of the necessary instructions for the
...
shortly.   Before attempting to install FLEX, the manuals should be read
...
keep all routines simple at first.       Once things are running in the
...
procedure, let's go through a brief summary of the steps involved.  The
...
doesn't care what kind of disk it is or where it is located.   The core
...
sector.   Thus the heart of the adaptation process is writing the
...
disk controller.    The development of these routines may be carried out
...
into object code.     In either case, it is convenient to have a mass
storage device on-line to save and load the drivers during development.

(2) Once the drivers are written, they must be fully tested.   A program
...
resident, user supplied drivers.   Once this FLEX is in memory and
...
Drivers.   Anytime FLEX must perform input or output to the system
...
will merely point out where these interrupt routines are located.   Full
...
$D3E4.   If the user's driver routines do not fit in this space, the
overflow will have to be placed somewhere outside the 8K FLEX area.  To
...
STAT      Address at $D3F7
          ...
          returned.   No registers may be modified except condition
          codes.

TINIT     Address at $D3F5
...
timer control for printer spooling.    For now these routines should
simply be disabled.      The three timer control routine vectors (TMINT,
...
Console Drivers found in Appendix G.    The only change that may be
...
in your system's monitor ROM.      If those routines fully meet the
...
Package.   The main body or core of FLEX is totally isolated from the
...
disk hardware.    Since the disk hardware can vary from installation to
installation, the user must supply these disk driver routines for his
particular   system.   They control the very basic, low-level disk
...
There is memory set aside for the drivers from DE00 to DFFF hex.      If
...
routine space.    The individual routines can be placed anywhere, but in
...
the area from $DE00 to $DE1D. It appears as follows.
           ...
           *          DE00                   ORG $DE00
           DE00 7E XXXX    READ    JMP XXXXX   Read a single sector
           DE03 7E XXXX    WRITE   JMP XXXXX   Write a single sector
           DE06 7E XXXX    VERIFY JMP XXXXX    Verify last sector written
           DE09 7E XXXX    RESTORE JMP XXXXX   Restore head to track #0
           DE0C 7E XXXX    DRIVE   JMP XXXXX   Select the specified drive
           DE0F 7E XXXX    CHKRDY JMP XXXXX    Check for drive ready
           DE12 7E XXXX    QUICK   JMP XXXXX   Quick check for drive ready
           DE15 7E XXXX    INIT    JMP XXXXX   Driver initialize (cold start)
           DE18 7E XXXX    WARM    JMP XXXXX   Driver initialize (warm start)
           DE1B 7E XXXX    SEEK    JMP XXXXX   Seek to specified track
...
floppy disk.    Once these drivers are fully functional and FLEX is
...
errors are encountered.  FLEX  takes   care   of   this   operation
automatically.
...
overflow the routines into other areas.        As long as the jump table
points to the beginning of each routine, they can be placed anywhere in
memory.   Obviously, it would be best if the routines can be fit in the
...
FLEX's $DFFF upper limit.      If the third case is possible. there is
...
one complication.    FLEX has assembled code in the printer spooler area
and when FLEX is loaded, this code is loaded.    Thus if the user has
...
assembled to the same addresses.       For more information on using the
...
value be set. For our example, the new MEMEND should be $7EFF.       For
...
routines.   Before doing so, however, it would be wise to save the code
for all the routines onto some mass storage device such as cassette or
paper tape if available.      This will allow you to quickly reload the
routines should something go wrong which wipes out memory.     The user
...
utility commands and, of course, the core of FLEX itself.     Hopefully,
...
writing when they should      not.    8"   and   5 1/4"   floppies   are
write-protected   in   different   ways.    The   8 inch floppies are
...
write-protected.   5 1/4 inch floppies are just the opposite of the 8
...
testing and we will refer to it as the "Scratch Disk".     Some of the
...
DRIVE     The Drive Select routine can probably be tested with no disk
          installed whatsoever.    To be sure, however, it is suggested
          that a scratch disk be installed during the test.         This
          routine is easy to test if the disk drives in use have LED's
          or lights which indicate the drive is selected.   If this is
          the case, simply write a little routine which calls the DRIVE
          routine with the proper entry parameters (see section 4.2) and
          then returns to your monitor.       If the routine functions
          ...
          this routine will be much more difficult to test.   You might
          just try calling it and being sure it returns properly. If
          so, assume it is working. If it is not, you will find that
          out as we proceed.

RESTORE   The Restore routine is a relatively easy routine to test.   It
          should be tested with a scratch disk installed in the drive
          and the door closed.    Before a restore operation can be
          performed on a drive, the desired drive must be selected by
          the DRIVE routine.   Thus to test RESTORE, write a short
          ...
          outlined in section 4.2.      If the    RESTORE   routine   is
          ...
          RESTORE   command   after one restore, it is possible to
          physically move the head out from track zero.    To do this,
...
SEEK      The SEEK routine must be tested with a disk installed. The
          ...
          on.   For example, if the controller track register says it is
          ...
          without subsequent restores.     So to test the SEEK routine,
          ...
          the disk.    Test this routine fully to see that-it seeks
...
paramters.    As a convenience for testing, however, we have provided the
listing for a short single sector test utility appropriately called
"TEST".    This assembled source listing is found in Appendix C. Using
your system's monitor ROM or whatever means you have, enter the code
listed for this program.       TEST assumes that all the Disk Driver and
...
This is a prompt for the desired drive number (a single digit from 0    to
3). After entering a drive number you should be prompted with:

          T?

Thi   is a prompt for a two-digit, hexadecimal track number. You can
...
which is a prompt for a two-digit, hexadecimal sector number.          Any
sector number may be given since an error should be returned if        the
...
hexadecimal.   There will be 16 rows of 16 bytes each. This display can
...
the disk.    You can be certain you have read the correct sector in most
cases by looking at the first two bytes of the data.   In most sectors
...
the WRITE routine.     If this routine runs wild, portions of data on a
...
possibly setting up the controller chip or DMA device).   If the WRITE
routine does fail and that failure causes indiscriminate writing to the
disk, chances are that only one track will be destroyed.    Thus before
...
to write the data to the disk.      If the write procedure functions
properly and there are no errors, TEST will print an "OK" on the screen
and start all over by prompting for another command.  If errors occur
...
avoided as follows.     There are three sectors on track zero which are
...
if it really would report an error.       It is recommended that you
...
into its place in memory.       The program is called 'QLOAD' for Quick
Loader and is listed in Appendix D.    The code for QLOAD should be
...
Drivers and the Console I/O Drivers.        Since we already have these
drivers in memory, we need only load FLEX.COR by using QLOAD in order to
run our first version of FLEX.

Once the code for QLOAD has been entered, write-protect a    FLEX Disk,
...
seconds to read.     If QLOAD does not perform as described, reload your
...
is in the system, and then prompts for the date.   After the date has
been entered, the disk in drive #0 is scanned for a file called
"STARTUP.TXT" as explained in the FLEX User's Guide.       There is no
...
are now ready to use FLEX.      The first tests should only involve
operations which perform reads from the disk.         Do not attempt any
...
down and begin executing commands which perform reads.  Some of these
commands are CAT, ASN, DATE, LIST, TTYSET, and VERSION. For proper
syntax and use of these commands, read the FLEX User's Guide.   To use
the LIST command you might try the following:

          +++LIST 0.ERRORS.SYS

This should list    the   system   error file which contains all of FLEX's
error messages.


6.3 Testing FLEX with Write Commands

Now you are ready to use FLEX to write information on the disk.  Remove
...
this version of FLEX in the remainder of the adaptation process.     The
...
or console drivers.     The final version of FLEX on a disk which may be
"bootstrap loaded" must also contain the disk and console driver
routines.   In this section we will create a new file on the disk called
...
Next, save your Console I/O Driver routines          in   a   file   called
...
execution of it.      This type of program is referred to as a "bootstrap
loader" since the system is essentially "pulling itself up by its
bootstraps".    Once this bootstrap loader has been developed, it can be
used to load FLEX. However, we still have the same problem - how do we
get   the    bootstrap loader into the computer after powering on?
...
memory and then jump to the beginning of the loader.    One thing that
...
be located anywhere outside the 8K reserved for FLEX.        It may be
...
That is the function of the LINK command found in FLEX.    It looks up
...
Package.   It is called with the track and sector numbers in 'A' and 'B'
and the address of where to read the data into memory in 'X'.          A
...
there was no error, the routine should return an EQUAL status.      Note
...
sector, you can skip this section completely.      If not, you should
...
FLEX has its own defined format for diskettes.      All disks must be
...
initialized with the FLEX format before use.         This initialization
...
ground up.    The NEWDISK routine is not a simple one and may take
...
9.1.1 Formatting the disk with 256 byte sectors.
          ...
          routine is also where all the sector linking takes place.   As
...
## PDF page 39
          ...
          track number $01 and sector number $02.   If a disk has 15
          ($0F) sectors on each track, the last sector on track #1
          (sector #15) should have $02 and $01 as its first two data
          bytes.   This means the next sector in the chain is on track
          ...
          "sector interleaving" in this formatting step.    See section
...
9.1.2 Testing and removing bad sectors.
          ...
          errors. If there are no errors, this step is complete.      If
...
9.1.3 Establishing the end of the free chain.
          ...
          The   end of the free chain of data sectors is easily
...
9.1.4 Initializing the directory.
          ...
          ends with the last sector on track #0.      This step should
...
9.1.5 Setting up the System Information Record (SIR).
          ...
          is the actual number of the last track.   For example, there
...
resident in that memory area when NEWDISK is executed.      For testing
...
disk, it makes no difference what is on track #0 sector #1.    When you
...
Digital NEWDISK found in Appendix F as a guide.      There is a large
...
actually does the disk formatting as described in section 9.1.1.      It
...
NEWDISK.   The second section performs steps 2 through 6 as described in
...
being able to read all the sectors in one revolution of the disk.    It
...
A simple solution to this problem is sector interleaving.   This refers
...
read head.    Thus logical sector number 2 may be put in physical sector
number 6. The logical sectors are thus "interleaved".

The distance (number of physical sectors) between logical sectors for
maximum performance is dependant on several factors.      These factors
...
different times for FLEX to process the data.          The distance or
...
system. In effect, this is a dedicated multi-tasking operation.    There
...
task so that both appear to be occurring simultaneously.   This section
...
interrupts produced must be IRQ type interrupts.   This timer must be
...
recommended value is 10 milliseconds.       If the printer in use is a
...
TMINT     Address at $D3F1
          This routine performs any necessary initialization for     the
          interrupt timer used by the printer tpooling process.      Any
...
IRQVEC    Address at $D3EB
          ...
          specified by the IRQ vector.       This IRQ vector location
          (address) should be placed in the Console I/O Driver vector
          table.

SWIVEC    Address at $D3E9
          ...
          location specified by the SW13 vector.      This SWI3 vector
          location (address) should be placed in the Console I/O Driver
          vector table.

IHNDLR    Address at $D3E7
          The Interrupt Handler routine is the one which will be
          executed when an IRQ interrupt occurs.        If using printer
          spooling, the routine should first clear the         interrupt
...
task.   For example, if we are currently running the printer task (the
...
on a single-sided disk, the drivers should select side #0.   If greater
...
Double-density   disks   are   usually   not   really   different   from
single-density disks with the exception of the fact that there are more
sectors per track.     Technical Systems Consultants has altered this
concept slightly.    In our specifications, a "double-density disk"
...
Anytime the drivers are accessing a sector on track           #0,   they
...
track of what density they think each drive is.   This will result in
...
fits together.    Somewhere in the drivers will be a set of four bytes
...
current drive is switched to the opposite density.   This read routine
...
to select.    For a double-sided, double-density eight inch disk system,
...
The correct check is chosen depending on the density in use.    In this
...
using all these configurations.      Two areas which must be altered for
...
for each type drive.      For example, suppose we have two eight inch
...
to each type disk drive.       A Winchester hard disk, for example, will
...
time of day clock). This version of FLEX provides this ability.    There
is a subroutine in the FLEX initialization code which displays the
prompt, obtains the response, and stores it in FLEX.   A call to this
...
subroutine.   If the date prompt is to be eliminated, one may simply
...
as the printer spooler code area.        If the user does not intend to
implement printer spooling in his system, some of this space may be used
for other purposes.     In particular, the area from $C71C through $C83F
...
presently stored in this area by altering the jump table.     This jump
...
terminated with a return (RTS).       Thus, it is possible for us to
...
The overlay code to disable the printer spooler section code might   look
something like this:
...
or lower case.     When the GETFIL routine (see the FLEX Advanced
...
by merely changing the value stored in MAPUP at $CC49 to $FF.       This
...
supplied in this package.      Replacement disks can be obtained from
...
format.   These are available in either 8 or 5 1/4 inch single-sided,
...
full of 6800 and 6809 related FLEX articles.          This newsletter is
...
its software packages are being installed.   When you get FLEX up and
running,   drop   us a line and let us know about your hardware
...
FLEX Disk Operating System.      Technical Systems Consultants Inc. has,
...
sector IBM format.    With the exception of track #0 which is in
...
from one system to another.    This standard allows the exchange of
...
formats are as follows:
       ...
       1) 8" SINGLE-SIDED, SINGLE-DENSITY, SOFT-SECTORED DISK
          This disk should be comprised of 77 tracks (numbered      0   thru
...
described below.    These formats are in use in many installations, but
...
refer to Single and Double Density respectively.
   ...
   Disk Type    # of Tracks     One Side Total      One Side Total
...
NOTES:
         ...
         1) On   double-density   disks,   track   #0   is   formatted in
...
adaptation of FLEX.   This sample is the adaptation of FLEX to a
...
via an ACIA located at $E004.     SBUG also redirects all interrupts
through its own RAM vectors,in the area of $DFC0.

The MF-68 disk system to Which these adaptions apply is a single-sided,
single-density, dual drive minifloppy system.       The controller board
...
The source listings of all the code necessary to        adapt   FLEX   to   the
...
console.    As can be seen, these are written for an ACIA at location
$E004. The interrupt vectors (IRQVEC and SWIVEC) are simply those setup
by SBUG.    The interrupt timer routines for printer spooling assume a
...
delay), the jump vectors for the two point to the same routine.    This
...
at $7000.    The user will probably wish to reassemble the code into ROM
...
sector.   It may be noted that the,"read single sector" routine used is
almost identical to that prepared for the Disk Driver package.  If the
...
"BRA FORM26").    All the values given in the skeletal NEWDISK for
...
changed to 40 if the drives were capable of writing 40 tracks.      The
sector maps have been altered to reflect the number of sectors and
proper interleaving for a single-sided, single-density minifloppy.  The
only code really added to the skeletal NEWDISK is the Write Track
routine and the Bootstrap Loader routine.     You will note that the
...
sample.   The remainder of the routine is identical to that of the
