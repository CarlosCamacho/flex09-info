...
> Source: `dev/basic/RBASIC - BASIC Language - Source Code and Build Files.zip!Rbasic.PDF`  
...
cursor control. Should be standardised on ^A ($01).

       $010E.   The HOME key on some terminals homes the cursor to the
bottom-left of the screen instead of the top-left corner, and so an ESC-
sequence has been provided for, which will divert the cursor to its
correct position at the top-left.     This string should be adapted as
appropriate to your terminal, or the first byte ($1B) set to $04 if your
terminal already homes correctly.
