...
> Source: `misc/archive-series/disk/DISK32 - FLEX Archive Disk - Source Code and Build Files - Includes ADD, CCZ80, COMPRESS.zip!DISK32.DSK!COMPRESS.DOC`  
> Method: FLEX disk extraction

OCR and media-decoding errors may remain.

COMPRESS(1)	SK*DOS Programmer's Manual	COMPRESS(1)
	
	NAME
...
The -t (text) option is used in SK*DOS to tell compress

that the file is to be handled as a text file.	This

applies to the input file on compress and the output file

on decompression.	If you are only storing a file for use

later on an SK*DOS system this option should not be used.

Text files are stored in space compressed mode normaly,

with a CR as the end of line character.	Compressing and

uncompressing the file as binary will preserve it intact.

This option should be used if the file is being sent to

a non SK*DOS system for uncompression or if you are

uncompressing a file obtained from a foreign system.
...
the limit imposed at compile-time.	The SK*DOS
...
SK*DOS does not support UNIX-style file links.	As a
