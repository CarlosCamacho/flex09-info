...
> Source: `apps/dataman/DMANMAN2 - DataMan Database - Documentation and Examples.zip!DMANMAN2.DSK!DMAN-16.TXT`  
...
.LM55
.FI
The program uses the standard database input for the 'NAME', 'PASSWORD', and 'DRIVE' for the database being flagged.
...
Usually, the user will want to act on the 'COMPLETE' database.
But if a user has run this program before and has set the 'RECORD CODE' flag to 'DELETED'
for any record meeting the main logic set, the user can rerun the program and deal with these 'DELETED' records only or the
...
The user then enters the 'FIELD NUMBER' for the logic test.
...
This can be very useful when dealing with Zip Codes.
.sp
...
First, it should be noted that if you choose only 1 'CONDITION TO SET', it doesn't matter if you use 'AND' or 'OR' logic.
The results will be the same for either.
.sp
One problem crops up when the user is in the 'AND' logic mode.
If you have chosen 2 conditions to set in 'AND' logic and you choose the same data field for both conditions,
.ul
...
For instance let's say you have a database in the example used on page 7-45.
...
When done, the database would contain records flagged 'SELECTED' for any record that met either of these two
...
If you have problems with getting the answers you are looking for, try building a 'logic table' to check the action
desired.
.sp2
.nf
.lm10
...
.LM55
.FI
This type of program can be very confusing to a programmer who is not well versed in logic.
...
.sp
The 'WORK DRIVE' is set up in lines 1195 to 1245 and the master conditions are set up in lines
1255 to 1335.
The 'MAIN LOGIC' is set-up in lines 1345 to 1405.
.sp
If the data is alpha, lines 1415 to 1505 are used to set the actual conditions and input of comparison data.
For numeric and money data, lines 1515 to 1640 are used.
This complete 'MAIN LOGIC' set-up is in a 'FOR/NEXT' loop starting at line 1355.
This loop is set by the variable 'CN' which is defined in line 1325.
.sp
After all the 'CN' conditions are set, the actual data from the '.DAT' database file is loaded and checked
...
.sp
The 'TEMP.SCR' file is written in lines 1960 to 2045.
...
First, the number of conditions is set to a maximum of '4'.
...
The trick to this change would be to set the variable 'CS$(X)' equal to the value
...
I don't see this as a problem as I have never seen a numeric field longer than 15 digits and a money field would have to be about
