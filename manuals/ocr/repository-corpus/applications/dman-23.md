...
> Source: `apps/dataman/DMANMAN2 - DataMan Database - Documentation and Examples.zip!DMANMAN2.DSK!DMAN-23.TXT`  
...
The field to be used must be pre-sorted in ascending order, using the 'STDMAN' 'SORT' program, before this program is run.
...
.FI
.LM55
After the database 'NAME', 'PASSWORD', and 'DRIVE' number are entered	the 'FILE HISTORY' is listed.
...
Also, the field to be used must be pre-sorted in ascending order.
If it has not been, the program will not give correct information for some of the report.
Once the data field for use has been defined, the 'RECORD CODE' conditions are prompted for.
.sp
...
One line of 'HEADER' information can also be entered if desired.
...
After the needed information is entered in lines 1210 to 1320, the database '.DAT' file is read in lines 1330 to 1375.
...
The next section from lines 1420 to 1525 deal with finding the 'MEDIAN' of the database data.
First it is decided if the number of samples found in the database is odd or even and the number of samples into the database
