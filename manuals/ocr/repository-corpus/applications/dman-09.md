...
> Source: `apps/dataman/DMANMAN2 - DataMan Database - Documentation and Examples.zip!DMANMAN2.DSK!DMAN-09.TXT`  
...
.LM 55
.FI
This program module will create a new database either from scratch or using an existing database as a model.
...
It can be chosen with a 'CR' without entering the choice number.
With the default choice, the user creates a new database from scratch.
.sp
The user first defines the disk drive for the database.
In a two drive system, this is usually drive 1.
It can be drive '0' if there is room on the DATAMAN system disk, but this is not recommended.
The user can, of course, take out the system disk and replace it with a formatted disk to put the database on.
If the user has done this the DATAMAN system disk will not be ready when the program module is done.
.sp
Using up to 8 characters, which conform to FLEX file name specifications, the user enters the database file name.
Then the password to protect the database is given.
The password must be at least one character and not more than 10 characters long.
...
The maximum number of fields allowed in a database is a function of the amount of program memory installed in your
computer.
.sp
For each field, the field name, maximum characters of data, and the data type are entered.
The field name can be of any length up to BASIC's input buffer limit, but should be kept under 29 characters
if at all possible.
...
If negative numbers are possible, add one more space for the minus sign.
...
if no negative money figures are to be used).
.sp2
.NF
.LM 22
9.1.2
...
.FI
.LM 55
The 'TYPE' of data is then entered.
...
embedded in the number make it a Type '1'.
...
of the database to be used as a model is entered.
The new name, drive number, password, and file description is then given for the new database.
The new database now has the same number of fields, the same field labels, data types, and data lengths as the database used as the
model.
.sp2
.lm 22
.nf
...
.LM 55
.FI
The field editor allows the user to edit any one of the 'FIELD LABELS', 'TYPE', or 'LENGTH' of data, but does not allow
...
.LM 55
.FI
The time and date is now entered.
This will be the time and date of 'FILE CREATION'.
This information will also be used as the 'LAST EDIT' time and date until the database is edited by a DATAMAN program
module.
.sp2
.lm 22
.nf
...
.FI
.LM 55
When building a new database, at least one record must be entered so the '.DAT' file can be built.
Up to this point, the '.SPC' file information has been prompted for.
.sp
For each 'FIELD LABEL', the data is now entered.
...
Any printable characters are permitted to be used in alpha data.
.sp
...
If negative data is to be entered, the '-' sign needs to precede the data.
.sp
...
The dollar sign and any commas will automatically be inserted when reports are printed by the 'RPWDMAN' program.
.sp
Data input is checked to make sure that no commas are used and that data does not excede the maximum characters defined
for the field data being entered.
There is no error checking done at this time for alpha characters inserted in numeric or money fields.
This error checking is done in the 'VERIFY DATABASE' program module.
.sp
There are times when no data is to be entered for a field in a record.
DATAMAN allows empty fields by simply hitting a 'CR' without any data.
Data
.ul
will be
entered into the database field however.
If the empty field is a 'TYPE' '1' or alpha, a 'NA' will be automatically inserted into the data field by DATAMAN.
...
.sp
If an error is made when entering data before the 'CR' is hit, use the backspace character defined ('CTRL H').
If an error is made and you catch it after the 'CR' has been hit and you are ready to enter the next field data,
...
When you have entered all the data fields in a record, you will be given the chance to edit the last field.
...
of a record, any data entered to that record will of course be lost.
.PG
.SP
.LM 10
.NF
...
.FI
.LM 55
This program is rather straightforward and the listing is documented well enough to make it easy to follow.
There are, however, a few items worth mentioning.
.sp
The first part of the program is a little different from other DATAMAN programs, in part due to the fact that the
program creates a database while most of the other program modules act on existing databases.
...
You could also use the FLEX/BASIC 'DATE$' for the date, but I found that sometimes it was convenient to
...
This delay was felt to be too long for a data input section where a fast 'key punch' operator could easily overrun the
...
.sp
During feedback from test sites, the suggestion was made to allow commas in alpha data.
...
Other changes might also be necessary.
.sp
