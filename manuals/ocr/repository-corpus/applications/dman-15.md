...
> Source: `apps/dataman/DMANMAN2 - DataMan Database - Documentation and Examples.zip!DMANMAN2.DSK!DMAN-15.TXT`  
...
The 'NAME' and 'DRIVE' of the format file is first entered.
The program gives the database name the format file was written for.
The user can then either use this database or request one of another name as long as the databases have
identical field labels in the same order.
...
.sp
When making the format file, the user defined the top two lines of the report 'HEADING'.
...
If an alternate database is used for the report, the name and drive are gotten in line 1275 to 1300.
...
The image data is located at line 1640 to 1665.
Up to 13 digits of money data can be formated.
If more digits are needed, simply add to the 'DATA' statements at line 1665.
.sp
...
The summation 'TOTALs' page are at lines 1865 to 1910.
.sp
.sp
.lm22
.nf
...
Some printers stay in the expanded print mode until reset with another type size.
...
In this program it occurs at line 1605.
...
.FI
.LM55
A similar change would be made in the 'RPEDMAN' program.
