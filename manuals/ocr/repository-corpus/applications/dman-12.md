...
> Source: `apps/dataman/DMANMAN2 - DataMan Database - Documentation and Examples.zip!DMANMAN2.DSK!DMAN-12.TXT`  
...
.FI
.SP3
This program is not really a sort program, rather it is an editor which sets up a 'procedure' file that is used by
the TSC 'SORT/MERGE' package.
Up to 20 keys can be defined from a DATAMAN database to be sorted.
Both ascending and descending sorting orders are supported.
.sp2
.lm 10
.nf
12.1.0
...
.FI
.LM 55
To sort a database the 'NAME', 'PASSWORD', and 'DRIVE' number of the database are entered.
...
.sp
The database fields are then listed and the user enters the number of 'KEYS' he wishes to sort on.
The program then enters the 'FIELD \#' for each of the 'KEYS' and the order that 'KEY' is to be sorted (ascending or descending).
...
In any case, a sorted database will be the same size as the	original database.
Also, the complete database is sorted, no matter what the 'RECORD CODE' of each record is.
.sp
The program then gives you the option of 'EXITing' the program, which will end all operations and take you
back to the 'MASTER MENU'.
You can 'REDEFINE' the sort, which takes you all the way back to the beginning of the program.
...
Any time error messages occur under the TSC header, they are TSC 'SORT/MERGE' error messages.
These error messages can be found in	the TSC 'SORT/MERGE' manual.
If you have defined the 'SORTED' database to replace the original database, a message will appear asking if you want to
...
Further information on this backup or '.BAK' file is given in the 'SORT/MERGE' manual, but you must answer 'Yes' to this prompt
...
sort is usually limited by the speed of the disk drives on your system.
.sp2
.lm10
.nf
12.2.0
...
run a machine language program, execute some FLEX utilities, the reload BASIC and run a BASIC program.
The real guts of this program is located in lines 1545 to 1685.
.sp
After the database fields are chosen for the sort 'KEYS', a 'CSORT' command line is built which will
