...
> Source: `apps/dataman/DMANMAN2 - DataMan Database - Documentation and Examples.zip!DMANMAN2.DSK!DMAN-10.TXT`  
...
.LM 55
.FI
The DATAMAN editor is used for 'file maintenance.
Field data can be found and changed and records can be added to a database.
As delivered, this program cannot delete or remove entire records from a database.
...
.sp
After the database 'NAME', 'PASSWORD', and 'DRIVE' number are entered and the 'FILE HISTORY' is given, the 'WORK DRIVE' is
prompted for.
.SP2
.LM 22
.NF
...
.sp
It is not too hard to get a rough guess of the size.
...
Second, you could move the database to an empty, but formatted, disk.
...
When I do this, I call the new disk 'DMANWORK'.
...
This operation is explained further in section 25.1.3.
...
things will happen.
First, a number of FLEX prompts will occur asking if files can be 'DELETED'.
If no disk error messages have shown up, answer 'YES' to all these prompts.
If error messages have been given, it is a good idea to answer these prompts with a 'NO' so that no work files are
deleted until the cause of the disk errors are known.
Because DATAMAN uses these work files, it is very rare that a complete database gets destroyed for any reason.
The second thing to note is that only one edit operation will occur before the editor loads the 'MASTER MENU' program.
In the example in section 7, you will note	that it is possible to do a 'LABEL/CONTENT' edit, then do an 'APPEND' edit, another
'LABEL/CONTENT' edit, etc.
...
.LM55
.FI
This editing mode is quite straightforward.
In this mode, you look for a certain record to edit by giving the program the field label number and the data contents of that field.
If the record is found, it is listed, and the user can change any or all the data in that record.
When you are done editing that record simply type a 'CR' and the next record (if any) which matches the label number and data
will be listed and be availabel for editing.
.sp
After each record has been edited, a prompt will occur asking if you wish to 'REDEFINE' the 'LABEL/CONTENT' search.
This can save a lot of time when a lot of editing is to be done.
Here is how it works.
.sp
Let's say you have a database with the 'LAST NAME' as one of the field labels.
...
.sp
You would first sort the database with the 'SORT DATABASE' program on the field 'LAST NAME' in ascending order.
You would also sort the 12 names you need to change in ascending order.
...
When you are asked for the first 'LABEL/CONTENT' for editing, you would type '1,ALBERT'.
...
When the 'LABEL/CONTENT' prompt comes up again, you type '1,CAPPER'.
This record is found and edited.
You continue to redefine the 'LABEL/CONTENT' until all 12 names are found and the records are edited.
This will save a lot of time when editing database that are large because the same work file is used for the
entire edit.
If you answered 'N' to the 'REDEFINE' prompt, the entire work file would be rebuilt each time a new name was entered for the
'LABEL/CONTENT' search.
.sp
You can also look at all the records in a database by typing a '1,!!' for the 'LABEL/CONTENT' prompt.
You can then edit any or all of the data fields in all the records of the database.
.sp
In the 'LABEL/CONTENT' editing mode you can restrict the search to only records that have the 'RECORD CODE FLAG' set 'SELECTED',
...
.LM55
.FI
If you choose to 'APPEND' or add additional records to a database, the operation is the same as when you added records
...
This can be very useful when you have a number of databases to update and you can't remember when they were last
updated.
By writing this date and time on source documents used while editing, you can create a good 'audit trail'
...
.LM55
.FI
The 'ETDMAN' program is basically broken into four parts.
The first section from line 1055 to 1260 sets up the database to be edited and defines the 'WORK DRIVE'.
This section is very similar to othe DATAMAN program modules.
.sp
The user can choose to edit the database by two methods.
If you choose the 'LABEL/CONTENT' edit mode, program lines 1270 to 1450 are used to set up the edit and lines
...
'.DAT' database file.
.sp
The final major section is the file 'clean-up' and the writing of a new '.SPC' file on the database disk.
...
.FI
.LM55
One change to this program that warrants some consideration is the addition of a 'RECORD CODE' edit.
This would allow the 'RECORD CODE' to be edited while in the 'LABEL/CONTENT' mode.
The only program that allows the user to change the 'RECORD CODE' is the 'SRFDMAN' program module.
If this feature were added to this program, it might be very useful under some conditions.
...
This change will allow the user to change the 'RECORD CODE' of any record found in the 'LABEL/CONTENT' search.
.sp
Changes in the 'FILE APPEND' section beginning at line 1605 is discussed in section 9.2.0.
.sp2
.nf
.lm 10
10.3.0
